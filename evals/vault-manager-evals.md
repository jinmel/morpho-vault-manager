...
| `OBS-BOOT-PARSER-001` | `parseOwsWalletCreateOutput` extracts walletRef, address, and mnemonic from happy-path output | Valid `ows wallet create --show-mnemonic` stdout | Parsed object contains walletRef, walletAddress, and mnemonic | `scripts/check/evals --only=OBS-BOOT-PARSER-001` |
| `OBS-BOOT-PARSER-002` | `parseOwsWalletCreateOutput` rejects output missing the mnemonic block | `ows wallet create` stdout without mnemonic section | Returns error indicating missing mnemonic block | `scripts/check/evals --only=OBS-BOOT-PARSER-002` |
| `OBS-BOOT-PARSER-003` | `parseOwsWalletCreateOutput` rejects output missing the "Created wallet" line | `ows wallet create` stdout without created wallet line | Returns error indicating missing created wallet line | `scripts/check/evals --only=OBS-BOOT-PARSER-003` |
| `OBS-BOOT-PARSER-004` | `parseOwsWalletList` extracts names and wallet refs from multi-entry listings | Multi-entry `ows wallet list` stdout | Returns array with name and walletRef for each entry | `scripts/check/evals --only=OBS-BOOT-PARSER-004` |
| `OBS-BOOT-PARSER-005` | `parseOwsWalletList` returns an empty array on empty stdout | Empty `ows wallet list` stdout | Returns empty array without error | `scripts/check/evals --only=OBS-BOOT-PARSER-005` |
| `OBS-BOOT-PARSER-006` | `parseOwsKeyCreateOutput` extracts the `ows_key_...` token | Valid `ows key create` stdout | Returns token matching `ows_key_...` pattern | `scripts/check/evals --only=OBS-BOOT-PARSER-006` |
| `OBS-BOOT-PARSER-007` | `parseOwsKeyCreateOutput` rejects output with no token | `ows key create` stdout without token | Returns error indicating no token found | `scripts/check/evals --only=OBS-BOOT-PARSER-007` |
