...
export type TokenSource =
  ...
  | { kind: "file"; path: string; mode?: "singleValue" | "json"; jsonField?: string }
  | { kind: "inline"; origin: string };

export type TokenResolution =
  | { ok: true; value: string; description: string }
  | { ok: false; error: string; description: string };

const INLINE_TOKEN_STORE = new Map<string, string>();

export function registerInlineToken(origin: string, value: string): void {
  INLINE_TOKEN_STORE.set(origin, value);
}

function describeSource(source: TokenSource): string {
  if (source.kind === "env") return `env:${source.envVar}`;
  if (source.kind === "inline") return `inline:${source.origin}`;
  if (source.mode === "json") return `file:${source.path}#${source.jsonField ?? "apiKey"}`;
  return `file:${source.path}`;
}

export async function resolveApiToken(source: TokenSource): Promise<TokenResolution> {
  ...
  try {
    ...
    return {
      ok: false,
      description,
      error: `Failed to read secret file ${source.path}: ${(error as Error).message}`
    };
  }
}

export function tokenSourceFromPluginConfig(
  value: unknown,
  fallbackEnvVar: string,
  options: { inlineOrigin?: string } = {}
): TokenSource {
  if (typeof value === "string" && value.trim().length > 0) {
