...
export async function upsertCronJob(params: {
  ...
  const baseArgs = [
    ...
    "--message",
    [
      `Run the Morpho vault rebalance for profile ${params.profile.profileId}.`,
      `Start with: openclaw vault-manager dry-run --profile ${params.profile.profileId} --json`,
      `If the dry-run status is "planned", continue with: openclaw vault-manager live-run --profile ${params.profile.profileId} --allow-live --json`,
