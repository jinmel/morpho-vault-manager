...
import { getMorphoPositions, getMorphoTokenBalance, getMorphoVault, runMorphoHealthCheck } from "../lib/morpho.js";
...
function parseArgs(argv) {
    const args = { keep: false, format: "text" };
...
async function queryBaseUsdcVaults(baseSettings) {
...
async function writeValidationProfile(settings, vaults) {
    ...
    const profile = {
        ...
        policyId: "morpho-vault-manager-live-path",
        policyFile: path.join(settings.dataRoot, "policies", "live-path.json"),
        policyExecutable: path.join(settings.dataRoot, "policies", "live-path.mjs"),
...
async function safeStep(id, description, fn) {
    try {
...
async function main() {
    ...
    const steps = [];
    try {
        steps.push(await safeStep("preflight", "Preflight checks", async () => {
        ...
        steps.push(await safeStep("openclaw-version", "openclaw --version", async () => {
            const { stdout } = await execFileAsync("openclaw", ["--version"], { timeout: 10_000 });
            return stdout.trim().split("\n").slice(0, 2).join(" | ");
        }));
        steps.push(await safeStep("ows-version", "ows --version", async () => {
            const { stdout } = await execFileAsync("ows", ["--version"], { timeout: 10_000 });
            return stdout.trim();
        }));
        steps.push(await safeStep("morpho-health", "morpho-cli health-check", async () => {
            const healthy = await runMorphoHealthCheck(settings);
            if (!healthy)
                throw new Error("morpho health-check returned non-zero");
            return "operational";
        }));
        steps.push(await safeStep("gateway-status", "openclaw gateway status", async () => {
            const reachable = await openclawGatewayIsReachable(settings);
            if (!reachable)
                throw new Error("openclaw gateway status returned non-zero");
            return "reachable";
        }));
        let chosenVaults = [];
        steps.push(await safeStep("query-vaults", "morpho query-vaults (Base USDC)", async () => {
        ...
        steps.push(await safeStep("get-vault", "morpho get-vault for each selected", async () => {
        ...
        steps.push(await safeStep("get-positions", `morpho get-positions ${LIVE_VALIDATION_WALLET}`, async () => {
        ...
        steps.push(await safeStep("get-token-balance", `morpho get-token-balance USDC ${LIVE_VALIDATION_WALLET}`, async () => {
            const balance = await getMorphoTokenBalance(settings, "base", BASE_USDC_ADDRESS, LIVE_VALIDATION_WALLET);
            return `${balance.balance.value} ${balance.balance.symbol}`;
        }));
        let dryRunStatus = "";
        steps.push(await safeStep("dry-run-rebalance", "runRebalance dry-run against live Morpho", async () => {
        ...
        steps.push(await safeStep("live-refusal", "live-run refuses without arming", async () => {
        ...
        if (args.format === "json") {
            ...
            for (const step of steps) {
                process.stdout.write(`[live-path] ${step.status.padEnd(4)} ${step.id} — ${step.description}${step.detail ? ` (${step.detail})` : ""}\n`);
            }
            process.stdout.write(`[live-path] summary pass=${report.totals.pass} fail=${report.totals.fail}\n`);
        }
        if (failed.length > 0) {
            process.exit(1);
        }
...
main().catch((error) => {
    process.stderr.write(`[live-path] ${error.stack ?? error.message}\n`);
    process.exitCode = 1;
});
