...
import { FIXTURE_VAULT_A, FIXTURE_VAULT_B, FIXTURE_WALLET, makeFixtureRebalanceDeps } from "../lib/fixtures.js";
import { saveProfile } from "../lib/profile.js";
import { runRebalance } from "../lib/rebalance.js";
function parseArgs(argv) {
    const args = { keep: false, scenario: "planned", format: "text" };
    for (const arg of argv) {
        if (arg === "--keep")
            args.keep = true;
        else if (arg === "--scenario=no-op" || arg === "--scenario=noop")
            args.scenario = "no-op";
        else if (arg === "--scenario=blocked")
            args.scenario = "blocked";
        else if (arg === "--scenario=planned")
...
function makeSandboxSettings(root) {
    return {
        dataRoot: root,
...
async function makeSandboxProfile(settings, scenario) {
    ...
    const profile = {
        ...
        policyId: `sandbox-policy-${scenario}`,
        policyFile: path.join(settings.dataRoot, "policies", `${scenario}.json`),
        policyExecutable: path.join(settings.dataRoot, "policies", `${scenario}.mjs`),
...
function depsForScenario(scenario) {
    if (scenario === "no-op") {
...
async function main() {
    ...
    try {
        ...
        const expectedStatus = args.scenario === "planned"
            ? "planned"
            : args.scenario === "no-op"
                ? "no_op"
                : "blocked";
        if (result.status !== expectedStatus) {
            throw new Error(`Expected sandbox scenario ${args.scenario} to produce status ${expectedStatus}, got ${result.status}`);
        }
    }
    finally {
        if (!args.keep) {
...
main().catch((error) => {
    process.stderr.write(`[sandbox] ${error.stack ?? error.message}\n`);
    process.exitCode = 1;
});
