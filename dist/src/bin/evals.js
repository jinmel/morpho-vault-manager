...
import { describeTokenSource, registerInlineToken, resolveApiToken, tokenSourceFromPluginConfig } from "../lib/secrets.js";
...
function fixturePositions(walletAddress, positions) {
...
function fixtureBalance(walletAddress, amount) {
    return {
        chain: "base",
        userAddress: walletAddress,
        asset: {
...
function fixtureDepsWithSimulationFailure(scenario) {
...
async function materializeProfile(settings, scenario) {
    const walletAddress = scenario.profile.walletAddress ?? SCENARIO_WALLET;
...
function assertContainsReason(result, substring) {
    const match = result.reasons.some((reason) => reason.includes(substring));
    if (!match) {
        throw new Error(`expected a reason containing "${substring}", got [${result.reasons.map((reason) => JSON.stringify(reason)).join(", ")}]`);
    }
}
function makeTempSettings() {
    const dataRoot = path.join(os.tmpdir(), `vault-manager-eval-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    return {
        dataRoot,
...
const SCENARIOS = [
    ...
    },
    {
        ...
        expect(result) {
            ...
            const totalAction = result.actions.reduce((acc, action) => acc + toUsdc(action.amountUsdc), 0n);
            const expectedMin = toUsdc("4800");
            if (totalAction < expectedMin) {
                throw new Error(`expected total action >= ${formatUnits(expectedMin, USDC_DECIMALS)} USDC, got ${formatUnits(totalAction, USDC_DECIMALS)}`);
            }
...
async function runScenario(scenario) {
    ...
    const deps = scenario.id === "REB-004"
...
async function runPolicyExecutable(executablePath, context) {
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            try {
                ...
                reject(new Error(`invalid policy output: ${parseError.message}\n${stdout}`));
            }
        });
...
function encodeCallData(selector, words) {
    return `${selector}${words.map((word) => word.padStart(64, "0")).join("")}`;
}
function addressAsWord(address) {
    return address.slice(2).toLowerCase().padStart(64, "0");
}
function uintAsWord(value) {
...
async function buildPolicyEnvironment() {
    const settings = makeTempSettings();
...
const POLICY_SCENARIOS = [
    {
        ...
        async setup() {
            return {
                ...
                transaction: {
                    to: VAULT_ALLOWED,
                    data: encodeCallData("0x6e553f65", [uintAsWord(1000000n), addressAsWord(SCENARIO_WALLET)]),
                    value: "0"
                }
            };
        },
        ...
        async setup() {
            ...
            return {
                ...
                transaction: {
                    ...
                    data: encodeCallData(approveSelector, [addressAsWord(VAULT_FORBIDDEN), uintAsWord(1000000n)]),
                    value: "0"
                }
            };
        },
...
async function runPolicyScenario(scenario) {
...
async function runSystemScenario(scenario) {
    await scenario.run();
}
...
function assertFalse(label, value, detail) {
    if (value) {
...
const SYSTEM_SCENARIOS = [
    {
        ...
        async run() {
            ...
            assertTrue("preflight.ok", result.ok, result.issues.map((issue) => issue.message).join("; ") || "no issues reported");
    ...
    },
    {
        ...
        async run() {
            ...
            assertTrue("api key references policy", apiKeyCommand.includes('--policy "morpho-vault-manager-default"'));
    ...
    },
    {
        ...
        async run() {
            ...
            assertTrue("gatewayReachable", reachable, "openclaw gateway status did not return success");
        }
    },
    {
        id: "CRN-002",
    ...
    },
    {
        ...
        async run() {
            const envVarSource = tokenSourceFromPluginConfig({ source: "env", provider: "default", id: "OWS_EVAL_ENV_TOKEN" }, "OWS_MORPHO_VAULT_MANAGER_TOKEN");
            assertEqual("envVarSource.kind", envVarSource.kind, "env");
            if (envVarSource.kind !== "env")
                throw new Error("unreachable");
            assertEqual("envVarSource.envVar", envVarSource.envVar, "OWS_EVAL_ENV_TOKEN");
            const fileSource = tokenSourceFromPluginConfig({ source: "file", provider: "mounted", id: "/run/secrets/morpho-token" }, "OWS_MORPHO_VAULT_MANAGER_TOKEN");
            assertEqual("fileSource.kind", fileSource.kind, "file");
            if (fileSource.kind !== "file")
                throw new Error("unreachable");
            assertEqual("fileSource.path", fileSource.path, "/run/secrets/morpho-token");
            const inlineSource = tokenSourceFromPluginConfig("oc-token-host-resolved-xyz-987", "OWS_MORPHO_VAULT_MANAGER_TOKEN", { inlineOrigin: "test:plugin-config:apiKey" });
            ...
            assertTrue("inlineResolution.description is inline", describeTokenSource(inlineSource).startsWith("inline:"));
        }
    }
];
function resolvePreflightSettings(inheritPath) {
    const settings = makeTempSettings();
    if (inheritPath) {
