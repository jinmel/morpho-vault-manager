...
import { createPublicClient, formatUnits, getAddress, http, parseSignature, parseTransaction, parseUnits, serializeTransaction } from "viem";
import { base } from "viem/chains";
import { MIN_ACTION_USDC, USDC_DECIMALS } from "./constants.js";
import { ensureDir, writeJsonFile } from "./fs.js";
import { getMorphoPositions, getMorphoTokenBalance, getMorphoVault, prepareMorphoDeposit, prepareMorphoWithdraw } from "./morpho.js";
...
function defaultReadDeps(settings, profile) {
    return {
        ...
        getTokenBalance: (walletAddress) => getMorphoTokenBalance(settings, profile.chain, profile.usdcAddress, walletAddress),
        prepareDeposit: (vaultAddress, walletAddress, amount) => prepareMorphoDeposit(settings, profile.chain, vaultAddress, walletAddress, amount),
        prepareWithdraw: (vaultAddress, walletAddress, amount) => prepareMorphoWithdraw(settings, profile.chain, vaultAddress, walletAddress, amount)
    };
}
...
function scaleAmount(value, numerator, denominator) {
    if (denominator === 0n)
        return 0n;
    return (value * numerator) / denominator;
}
function capBps(maxSingleVaultPct) {
    return BigInt(Math.round(maxSingleVaultPct * 10_000));
}
function targetCashBuffer(totalManaged, preset) {
...
function buildRankedCandidates(vaults, preset) {
    const rejected = [];
    const eligible = [];
    ...
    const ranked = eligible.map((candidate) => {
        ...
        const rewardsPenaltyTerm = dislikesRewards && candidate.apyPct >= rewardsPenaltyThreshold
    ...
    ranked.sort((left, right) => {
        ...
        if (right.apyPct !== left.apyPct)
            return right.apyPct - left.apyPct;
...
function allocateTargets(total, candidates, preset) {
    const targets = new Map();
    if (total <= 0n || candidates.length === 0)
        return targets;
    const cap = (total * capBps(preset.maxSingleVaultPct)) / 10000n;
    ...
    while (remainingIndices.size > 0) {
        ...
        active.forEach((index, position) => {
            const amount = position === active.length - 1
...
function plannedTurnover(actions) {
...
function scaleActionsForTurnover(actions, turnoverCap) {
    if (turnoverCap <= 0n)
        return [];
    const desired = plannedTurnover(actions);
    if (desired <= turnoverCap)
        return actions;
...
function capDepositsToAvailableCash(actions, currentIdle, targetIdle) {
    ...
    if (desiredDeposits <= available)
        return actions;
    if (available <= 0n)
        return actions.filter((action) => action.kind !== "deposit");
    ...
    return trimActions(actions
        .map((action) => (action.kind === "deposit" ? depositMap.get(action.vaultAddress) ?? action : action))
        .filter(Boolean));
}
function driftExceeded(totalManaged, targetIdle, currentIdle, currentAmounts, targetAmounts, preset) {
    if (totalManaged <= 0n) {
    ...
    for (const [address, targetAmount] of targetAmounts.entries()) {
        if (currentAmounts.has(address))
            continue;
        if (targetAmount > maxDrift)
            maxDrift = targetAmount;
    }
    const maxObservedPct = ratioPctString(maxDrift, totalManaged);
    const thresholdAmount = scaleAmount(totalManaged, BigInt(Math.round(preset.rebalanceDriftPct * 10_000)), 10000n);
    return {
...
function buildActionDrafts(params) {
    ...
    actions.sort((left, right) => {
        if (left.kind !== right.kind)
            return left.kind === "withdraw" ? -1 : 1;
        return right.amount > left.amount ? 1 : right.amount < left.amount ? -1 : 0;
    });
    return actions;
}
async function prepareActionOperations(profile, actions, deps) {
    const operations = [];
    for (const action of actions) {
        const amountUsdc = formatUsdc(action.amount);
        try {
            const prepared = action.kind === "deposit"
...
function toOperationResult(action, prepared) {
    return {
        kind: action.kind,
        vaultAddress: action.vaultAddress,
        vaultName: action.vaultName,
...
function resolveSignedTransaction(unsignedTransactionHex, payload) {
    ...
    const parsedUnsigned = parseTransaction(unsignedTransactionHex);
...
async function executeOperations(params) {
    ...
    const maxPriorityFeePerGas = fees.maxPriorityFeePerGas ?? fees.gasPrice ?? 1000000n;
    const maxFeePerGas = fees.maxFeePerGas ?? fees.gasPrice ?? maxPriorityFeePerGas;
    ...
    for (const operation of params.operations) {
        for (const tx of operation.transactions) {
            ...
            const transactionHash = signResult.payload.transactionHash ??
...
function receiptPathForRun(settings, profileId, runId) {
    return path.join(settings.dataRoot, "runs", profileId, `${runId}.json`);
}
function currentAmountsFromPositions(positions) {
    return new Map(positions.map((position) => [position.vault.address, toUsdcUnits(position.supplied.value)]));
}
...
export async function runRebalance(settings, profileId, mode, overrideDeps) {
    ...
    const allowedVaultResults = await Promise.allSettled(normalizedAllowedVaults.map((address) => deps.getVault(address)));
    const liveVaults = [];
    ...
    const managedPositions = positionsResponse.vaultPositions.filter((position) => allowedVaultSet.has(getAddress(position.vault.address)));
    ...
    const drift = driftExceeded(totalManaged, targetIdle, idleUsdc, currentAmounts, targetAmounts, profile.riskPreset);
    const vaultNames = new Map();
    for (const vault of liveVaults)
        vaultNames.set(vault.address, vault.name);
    for (const position of managedPositions)
        vaultNames.set(position.vault.address, position.vault.name);
    if (totalManaged === 0n) {
    ...
    if (totalManaged > 0n && !drift.exceeded) {
        reasons.push(`Current allocation drift (${drift.maxObservedPct}%) is below the configured threshold (${percentString(profile.riskPreset.rebalanceDriftPct * 100)}%).`);
    }
    ...
    let operations = [];
    let status = reasons.length > 0 ? "no_op" : "planned";
    if (status === "planned") {
        ...
        if (failedOperation) {
            status = "blocked";
            reasons.push(failedOperation.error ??
    ...
    const result = {
        ...
        warnings: [
            ...warnings,
            ...operations.flatMap((operation) => operation.warnings.map((warning) => `${warning.level}: ${warning.message}`))
        ],
        metrics: {
            idleUsdc: formatUsdc(idleUsdc),
            totalManagedUsdc: formatUsdc(totalManaged),
...
async function persistRunReceipt(result) {
