...
export function renderAgentInstructions(profile) {
    const preset = RISK_PRESETS[profile.riskProfile];
    const allowedVaultLines = profile.allowedVaults.length > 0
    ...
    const allowedSpenderLines = profile.allowedSpenders.length > 0
