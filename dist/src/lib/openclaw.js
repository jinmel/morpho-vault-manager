...
export async function openclawGatewayIsReachable(settings) {
...
export async function enableCronJob(settings, cronJobId) {
    const result = await runCommand(settings.openclawCommand, ["cron", "enable", cronJobId]);
    return result.code === 0;
}
export async function disableCronJob(settings, cronJobId) {
    const result = await runCommand(settings.openclawCommand, ["cron", "disable", cronJobId]);
    return result.code === 0;
}
export async function runCronJobNow(settings, cronJobId) {
...
export async function listCronJobs(settings) {
