...
export function resolveVaultManagerSettings(pluginConfig) {
    ...
    return {
        ...
        morphoCliArgsPrefix: (stringValue(config.morphoCliCommand) ?? "bunx") === "bunx"
            ? ["--package", "@morpho-org/cli", "morpho"]
            : [],
        baseRpcUrl: stringValue(config.baseRpcUrl) ??
        ...
        defaultTimezone: stringValue(config.defaultTimezone) ??
            Intl.DateTimeFormat().resolvedOptions().timeZone ??
            "UTC",
        defaultTokenEnvVar: stringValue(config.defaultTokenEnvVar) ?? "OWS_MORPHO_VAULT_MANAGER_TOKEN",
        defaultTokenSource: tokenSourceFromPluginConfig(config.apiKey, stringValue(config.defaultTokenEnvVar) ?? "OWS_MORPHO_VAULT_MANAGER_TOKEN"),
