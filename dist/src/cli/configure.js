...
function agentIdForProfile(settings, profileId) {
    return profileId === "default" ? settings.baseAgentId : `${settings.baseAgentId}-${profileId}`;
}
function workspaceDirForAgent(settings, agentId) {
...
async function promptWallet(existing, settings) {
    ...
    if (resolvedMode === "created") {
        ...
        await p.note(`Run this command in another shell, complete the OWS prompts, then return here:\n\n${buildWalletCreateCommand(settings, walletName)}`, "Create Wallet");
...
async function runValidationDryRun(settings, profileId) {
    ...
    try {
        ...
        await p.note([
            ...
            ...(result.actions.length > 0
                ? [
                    "",
                    "Planned actions:",
                    ...result.actions.map((action) => `- ${action.kind} ${action.amountUsdc} USDC via ${action.vaultName}`)
                ]
                : [])
        ].join("\n"), "Validation Dry Run");
        return result;
        ...
        await p.note(`Validation dry-run failed: ${error.message}`, "Validation Error");
        return null;
    }
}
export async function runConfigureFlow(context) {
    ...
    const vaultsInput = optionalString(await p.text({
    ...
    const spendersInput = optionalString(await p.text({
    ...
    const cronExpression = requiredString(await p.text({
...
async function presentRunResult(result, json, title) {
    if (json) {
...
export async function runProfileDryRun(settings, profileId, json) {
    const result = await runRebalance(settings, profileId, "dry-run");
    await presentRunResult(result, json, `Dry Run: ${profileId}`);
}
export async function runProfileLive(settings, profileId, json, allowLive) {
    if (!allowLive) {
